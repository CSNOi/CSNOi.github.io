...
1. **数据去中心化**：设数据集为 ![][image1]，计算均值并减去：  
   ![][image2]  
2. **计算协方差矩阵**：  
   ![][image3]  
3. **特征值分解**：  
   对 ![][image4] 进行分解，找到特征值 ![][image5] 和特征向量 ![][image6]：  
   ![][image7]  
4. **降维投影**：  
   选取前 ![][image8] 个最大特征值对应的特征向量组成矩阵 ![][image9]。降维后的数据 ![][image10] 为：  
   ![][image11]
...
1. **高维权边计算**：  
   ![][image18]  
   其中 ![][image19] 是点 ![][image20] 到其最近邻的距离。  
2. **低维权边模拟**：  
   ![][image21]  
3. **目标函数（交叉熵）**：  
   UMAP 同时关注“吸引”和“排斥”：  
